...
<script>
	...
	export default{
		...
		methods:{
			...mapMutations(['updateUserCreateList']),
			cancel(){
				this.$router.push({
		...
		computed:{
			...mapGetters(['userCreatedList','createdListIdArr']),
			changed(){
		...
		created(){
			...
			let index = this.createdListIdArr.indexOf(name.toString());
			let playlist = this.userCreatedList[index];
			this.name = name;
