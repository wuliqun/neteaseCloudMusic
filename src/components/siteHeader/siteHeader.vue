      ...
      <div class="wrapper">
        ...
        <ul class="navs">
          <li class="nav-item" v-for="(item, index) in siteStructure" :key="item.id">
            <router-link :to="{ name: item.routerName }" v-if="!item.isOuterLink" :exact="index === 0"
              :class="{ active: isDiscover && index === 0, hot: item.isHot }">
              {{ item.name }}
              <i v-if="item.isHot"></i>
            </router-link>
            <a :href="item.href" v-else :class="{ hot: item.isHot }">
              {{ item.name }}
              <i v-if="item.isHot"></i>
            </a>
          </li>
      ...
      <div class="wrapper">
        <ul class="discover-nav clearfix" v-show="isDiscover">
          <li class="discover-item" v-for="(item, index) in siteStructure[0].children" :key="item.id">
            <router-link :to="{ name: item.routerName }" :class="{ active: isRecomend }" exact v-if="!index">
              {{ item.name }}
            </router-link>
...
#header .nav-item a.router-link-active,
...
#header .nav-item a.active:after {
  ...
  width: 14px;
  height: 6px;
  background: url(../../img/topbar.png) no-repeat -215px -140px;
...
#header .nav-item a.hot i {
  ...
  width: 26px;
  height: 13px;
  background: url(../../img/topbar.png) no-repeat -192px 0;
}
...
#header .icon-login {
  ...
  height: 20px;
  width: 18px;
  background: url(../../img/toplist.png) no-repeat 0 0;
}
...
#header .discover-item a.router-link-active,
#header .discover-item a:hover,
